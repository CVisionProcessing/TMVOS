...
class COCO(data.Dataset):
    def __init__(self,base_dir=config.coco_datadir,img_dir =config.coco_datadir+"train2014",
        ...
        for c in classes:
            ...
            for i in img:
                ...
                self.spa_prior_path.append(os.path.join(base_dir,"Deformations",c,os.path.splitext(i)[0]+'_d'+str(tmp[0])+'.png'))
    ...
    def __getitem__(self, index):
        ...
        sample = {'image': _img, 'label': _target, 'first_image': _img_first,'spa_prior':_spa_prior,'target_first':_target_first,"spa_img":_img}#coco数据集没有前一帧的图片没办法训练
...
class DavisDataset_video1(data.Dataset):
    def __init__(self,
        ...
        for vc in self.vedio_classes:
            ...
            for i in range(len(frame_list)):
                ...
                self.label_path.append(os.path.join(config.davis_datadir+'Annotations/480p',vc,os.path.splitext(frame1)[0]+'.png'))#bug
                self.first_path.append(os.path.join(dataset_prefix,vc,frame2))
                self.first_mask.append(os.path.join(config.davis_datadir+'Annotations/480p',vc,os.path.splitext(frame2)[0]+'.png'))
                while(1):
                    n = random.sample([-3,-2,-1,1,2,3],1)
                    if os.path.exists(os.path.join(config.davis_datadir+"Annotations/480p",vc,str(int(os.path.splitext(frame1)[0])+n[0]).zfill(5)+'.png')):
                        self.spa_prior_path.append(os.path.join(config.davis_datadir+"Annotations/480p",vc,str(int(os.path.splitext(frame1)[0])+n[0]).zfill(5)+'.png'))
                        self.spa_img.append((os.path.join(dataset_prefix,vc,str(int(os.path.splitext(frame1)[0])+n[0]).zfill(5)+'.jpg')))
                        break
    ...
    def __getitem__(self, index):
        ...
        sample = {'image': _img, 'label': _target, 'first_image': _img_first,'spa_prior':_spa_prior,'target_first':_target_first,"spa_img":_spa_img}
...
class DavisDataset_video_val(data.Dataset):
            ...
            for c in range(1,vc_c+1):
                for frame in os.listdir(os.path.join(dataset_prefix,vc)):
                    if frame!="00000.jpg":
                        ...
                        self.label_path.append(os.path.join(config.davis_datadir+"Annotations_binary/480p",vc,os.path.splitext(frame)[0]+'_'+str(c)+".png"))
                        self.first_path.append(os.path.join(dataset_prefix,vc,"00000.jpg"))
                        self.first_mask.append(os.path.join(config.davis_datadir+"Annotations_binary/480p",vc,"00000_"+str(c)+".png"))
                        self.spa_prior_path.append(os.path.join(config.davis_datadir+'Annotations_binary/480p',
                            vc,str(int(os.path.splitext(frame)[0])-1).zfill(5)+'_'+str(c)+'.png'))
        self.transform = transform
    ...
    def __getitem__(self, index):
        _img, _target,_img_first,_spa_prior,_target_first= self._make_img_gt_point_pair(index)
        sample = {'image': _img, 'label': _target, 'first_image': _img_first,'spa_prior':_spa_prior,'target_first':_target_first}
        ...
        for seq in sequences:
            ...
            for vc in vedio_classes:
                for i in range(len(frame_list)):
                    ...
                    self.label_path.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,os.path.splitext(frame1)[0]+'.png'))
                    self.first_path.append(os.path.join(dataset_prefix,"JPEGImages",seq,frame2))
                    self.first_mask.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,os.path.splitext(frame2)[0]+'.png'))
                    '''
                    if np.where(frame_list==frame1)==0:
                        ...
                        self.spa_prior_path.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,os.path.splitext(frame_list[np.where(frame_list==frame1)-1])[0]+".png"))
                    '''
                    if os.path.exists(os.path.join(dataset_prefix,"JPEGImages",seq,str(int(os.path.splitext(frame1)[0])-5).zfill(5)+".jpg")):
                        self.spa_prior_path.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,str(int(os.path.splitext(frame1)[0])-5).zfill(5)+".png"))
                    elif os.path.exists(os.path.join(dataset_prefix,"JPEGImages",seq,str(int(os.path.splitext(frame1)[0])+5).zfill(5)+".jpg")):
                        self.spa_prior_path.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,str(int(os.path.splitext(frame1)[0])+5).zfill(5)+".png"))
                    else:
                        frame3 = random.sample(frame_list,1)[0]
                        self.spa_prior_path.append(os.path.join(dataset_prefix,"Annotations_split",seq,vc,str(os.path.splitext(frame3)[0]).zfill(5)+".png"))
                        self.spa_img_path.append(os.path.join(dataset_prefix,"JPEGImages",seq,str(int(os.path.splitext(frame3)[0])).zfill(5)+".jpg"))
                    '''
                    while(1):
